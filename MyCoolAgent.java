...
public class MyCoolAgent implements PlanningAgent {
	...
	private long compulsoryTimeToReserve = 1;		// must reserve this minimum amount (in ms) to stop DAS from running when there's no time left
	...
	public GridCell getNextMove(GridDomain map, GridCell sState,
		...
		if (planned) {
			...
			if (!goalState.equals(gState)) {
				...
				/* Did it move away? */
				double oldDistance = sState.getCoord().getManhattenDistance(
						goalState.getCoord());
				double newDistance = sState.getCoord().getManhattenDistance(
						gState.getCoord());
				if (oldDistance < newDistance) {

					/* Plan a path between those goal positions */
					startCell = closedList[goalState.getCoord().getX()][goalState
							.getCoord().getY()];
					...
					ComputedPlan goalPath = executeDAS(map, oldGoalCell,
							startCell, timeLeft, 0);
			...
			/* 2. Map has changed */
				...
				for (Edge edge : map.getChangedEdges()) {
					...
					for (int i = pathStepNo; i < bestPath.getLength(); i++) {
						...
						if (pivotState.equals(edge.getStart())
								|| pivotState.equals(edge.getEnd())) {
							/* Yes! Replan! */
							planned = false;
							firstRun = true;
							secondRun = false;
			...
			/* 3. Time was generously increased with none of the above accompanying causes */
				...
				startCell = closedList[sState.getCoord().getX()][sState
						.getCoord().getY()];
				double currentGoalF = incumbent.gUnweighted
						- startCell.gUnweighted;
				startCell.parent = null;
				initializeParameters(map, sState, gState, timeLeft);
				startCell.updateCosts(0, startHeuristic, 1, 1);

				/* Do DAS */
				bestPath = executeDAS(map, sState, startCell, timeLeft,
						currentGoalF);
		...
		if (!planned && firstRun) {
			...
			startCell = new GridCellInformed(sState, 0, startHeuristic, 0,
					weightG, weightH);
			...
			timeReserved += compulsoryTimeToReserve; // any necessary amount of time reservation?
		...
		if (!planned && secondRun) {
			...
			if(bestPath.getLength()<=1) System.exit(1);
		}
	 ...
	 * 
	 * @param currentCell
	 *            - start cell for the greedy algorithm
	...
	private GridCellInformed greedy(GridCellInformed currentCell) {
		...
		while (!currentCell.cell.equals(goalState)) {
			...
			SuccessorIterator childIterator = map
					.getNextSuccessor(currentCell.cell);
			...
			/* Which directions are preferred from the parent? */
			setOperatorPreference(currentCell.cell, goalState);

			while ((child = childIterator.next()) != null) {
				/* Already visited? Blocked? Self? Ignore */
				if (isClosed(child) || map.isBlocked(child)
						|| child.equals(currentCell.cell))
					continue;
			...
			currentCell = new GridCellInformed(nextCell, currentCell, map.cost(
					currentCell.cell, nextCell), hCost(nextCell, currentCell),
					0, weightG, weightH);
		}
	 ...
	 *            - start cell
	 * @param startCell
	 *            - (optional) start cell with costs already recorded
	 * @param timeLeft
	 *            - for establishing deadline for DAS
	 * @param maxF
	 *            - for subsequent DAS runs, you can tell it to find solutions
	 *            of no more than maxF cost
	 * @return full computed plan
	 */
	private ComputedPlan executeDAS(GridDomain map, GridCell sState,
			GridCellInformed startCell, long timeLeft, double maxF) {
		if (timeLeft > timeReserved) {
			...
			if (goalFound && (maxF <= 0 || incumbent.fUnweighted < maxF))
				return extractPlan(sState, incumbent);
		}
		return bestPath;
	}

	/**
	 * Core DAS algorithm. Solution path can be traced back from returned goal
	 * node using extractPlan()
	 * 
	 * @param startCell
	 *            - start cell with initial costs set
	 * @param deadline
	 *            - time in millis since Jan 1, 1970, until DAS can run
	...
	private GridCellInformed das(GridCellInformed startCell, double deadline) {
		...
		while (deadline - System.currentTimeMillis() > averageTimeTaken) {
			...
			if (startCell != null || !openList.isEmpty()) {
				...
				/*
				 * Get the next node from the open list or the supplied start
				 * node
				 */
				...
				if (incumbent != null
						&& currentCell.fUnweighted > incumbent.fUnweighted) {
					continue;
				}

				/* Is it a goal (and cheaper than incumbent)? */
				if ((currentCell.cell != null && currentCell.cell
						.equals(goalState))
					...
					eDelayWindow[eDelayWindowCurrPosition++] = eCurr
							- (int) currentCell.e;
					...
					while ((child = childIterator.next()) != null) {
						...
						if (isClosed(child)) {
							...
							GridCellInformed childCell = closedList[child
									.getCoord().getX()][child.getCoord().getY()];
							if (childCell.parent != null
									&& currentCell.fUnweighted < childCell.parent.fUnweighted) {
								unmarkClosed(childCell);
							} else
								continue;
						} else if (map.isBlocked(child)
								|| child.equals(currentCell.cell)) {
							continue; // skip
						}

						/* Add child to open list */
						GridCellInformed childCell = new GridCellInformed(
								child, currentCell, map.cost(currentCell.cell,
										child), hCost(child, currentCell),
								eCurr, weightG, weightH);
						openList.add(childCell);
					}

					/* Calculate time delay */
					tDelayWindow[tDelayWindowCurrPosition++] = System
							.nanoTime() - tPreExpansion;
	...
	/**
	 * Function for computing dmax - will not compute dmax (return infinity) for
	 * the duration of settlingTime
	 * 
	 * @param deadline
	 *            - allotted to DAS
	 * @return dmax value
	...
	/**
	 * Computes dCheapest. So far, doesn't get into the whole dCheapest_hat and
	 * dCheapest_error factors, for simplicity.
	 * 
	 * @param cell
	 *            - cell for which dCheapest must be calculated
	...
	private double dCheapest(GridCell cell) {
		...
		// double x = Math.abs(cell.getCoord().getX() -
		// goalState.getCoord().getX());
		// double y = Math.abs(cell.getCoord().getY() -
		// goalState.getCoord().getY());
	...
	/**
	 * DAS-specific function to recover pruned nodes. As a result, the open list
	 * will now have new nodes to consider.
	 * 
	 * @param deadline
	 *            - allotted to DAS
	 */
	private void deprune(double deadline) {
		...
		if (settlingTimeLeft > 0) {
			...
			if (weightH > startHeuristic)
				weightH = startHeuristic;
			...
			if (!goalFound && (weightH + weightG <= 2))
				weightH = weightG;
			else
				weightG = 1f;
	 ...
	 * 
	 * Sets operator preference - [North, East] where both directions are +1 if
	 * that direction is favoured -1 if not, and 0 if that direction is
	 * irrelevant. Direction is considered favoured if the goal is in that
	 * direction.
	 * 
	...
	private void setOperatorPreference(GridCell sState, GridCell gState) {
		...
		/*
		 * Set direction preferences by which quadrant the goal is in Quadrants
		 * (numbers = index of operatorPreference): 
		 * 	   0 	2 
		 * 	NW | NE
		 * 	-------	1
		 * 	SW | SE
		...
		if (xdiff >= 0 && ydiff >= 0) {
			/* Quadrant NE */
			operatorPreference[0] = 1;
			operatorPreference[1] = 1;
			if (connectivity == 8) {
				operatorPreference[2] = 1;
				operatorPreference[3] = 0;
			}
			...
			operatorPreference[1] = -1;
			if (connectivity == 8) {
				operatorPreference[2] = 0;
				operatorPreference[3] = 1;
			}
		} else if (xdiff < 0 && ydiff < 0) {
			/* Quadrant SW */
			operatorPreference[0] = -1;
	 ...
	 * Uses incremental heuristic computation. 
	 * I.e., heuristic child = heuristic parent + 1, or -1, or same, 
	 * based on whether the motion was towards the goal, away, etc. 
	 * Make sure to call setOperatorPreference to determine which direction the goal is compared to the parent cell! Suggestion: call
	 * it once before expanding children
	 * 
	 * @param cell
	 *            - cell whose h is needed
	 * @param parent
	 *            - cell's parent, for quick incremental heuristic computation
	...
	private double hCost(GridCell cell, GridCellInformed parent) {
		...
		/* Compute the direction we moved in */
		int north = (int) Math.signum(ydiff); // North
		int east = (int) Math.signum(xdiff); // East
		int leadDiagonal = 0;
		int lagDiagonal = 0;
		if (connectivity == 8) {
			leadDiagonal = (int) ((north * east != 1) ? 0 : Math.signum(xdiff));
			lagDiagonal = (int) ((north * east != -1) ? 0 : Math.signum(xdiff));
		}

		/*
		 * Weigh directions by their preference. 
		 * Make sure preference is set relative to the parent! 
		 * Use setOperatorPreference for the parent before calling this on function the children
		 */
		north *= operatorPreference[0];
		east *= operatorPreference[1];
		if (connectivity == 8) {
			leadDiagonal *= operatorPreference[2];
			lagDiagonal *= operatorPreference[3];
		}

		/* Total weighted change */
		double change = (north + east + leadDiagonal + lagDiagonal);
		/* Change must not be larger than 1 */
		change = Math.signum(change);

		/* Compute new heuristic by subtracting change from parent's h */
		double h = parent.h - change * minCost;
		return h;
	}
	 ...
	 *            - time left, what else?
	 */
	private void initializeParameters(GridDomain map, GridCell sState,
			GridCell gState, long timeLeft) {
		/* Node lists */
		...
		connectivity = getMapConnectivity(sState);
		/* Get goal direction */
		operatorPreference = new int[connectivity / 2]; // North & East
	 ...
	 * Example, returns 4 for Manhattan grids, 8 for Euclidean.
	 * Useful for custom heuristic implementation.
	 * Call it once at the start and use result as desired.
	 * 
	 * @param sampleCell
	 *            - usually the start or current cell
	 * @return connectivity (int)
	 */
	private int getMapConnectivity(GridCell sampleCell) {
		SuccessorIterator childIterator = map.getNextSuccessor(sampleCell);
		GridCell child;
		int connectivity = 0;
		while ((child = childIterator.next()) != null) {
			if (!child.equals(sampleCell))
				connectivity++;
		}
		return connectivity;
	}
	 ...
	 * 
	 * Handy function to trace back the path from the goal node. Make sure as
	 * you search, you set parent nodes for each child.
	 * 
	 * @param sState
	 *            - start cell
	 * @param goalNode
