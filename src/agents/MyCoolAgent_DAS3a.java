...
public class MyCoolAgent_DAS3a implements PlanningAgent{
	private ComputedPlan bestPath = new ComputedPlan();
	private TreeMap<SearchNode, Double> openList; // = new PriorityQueue<SearchNode>(100, new OpenListComparator());
	private TreeMap<SearchNode, Double> prunedList; // = new PriorityQueue<SearchNode>(100, new OpenListComparator());
	private TreeMap<SearchNode, Double> closedList; // = new PriorityQueue<SearchNode>(100, new ClosedListComparator());
...
class OpenListComparator implements Comparator<SearchNode>{
	public int compare(SearchNode me, SearchNode them){
		float myF = me.get(MyCoolAgent_DAS3a.F);
		float theirF = them.get(MyCoolAgent_DAS3a.F);

		int comparison = Float.compare(myF, theirF);

		/* Tie breaking - go with the lower heuristic */
		if(comparison == 0){
			myF = me.get(MyCoolAgent_DAS3a.H);
			theirF = them.get(MyCoolAgent_DAS3a.H);
			comparison = Float.compare(myF, theirF);
		}
		return comparison;
	}
}

class ClosedListComparator implements Comparator<SearchNode>{
	public int compare(SearchNode me, SearchNode them){
		float myF = me.get(MyCoolAgent_DAS3a.H);
		float theirF = them.get(MyCoolAgent_DAS3a.H);
