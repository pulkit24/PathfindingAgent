...
public class MyCoolAgent_RWA_G implements PlanningAgent{
		...
		while(!openList.isEmpty() && System.currentTimeMillis() < deadline){
			...
			for(State child:children){
				...
				if(!openList.contains(childNode) && !closedList.contains(childNode)){
					...
					float h = map.hCost(child, goalState);
					float f = currG + weight * h;
					childNode.set(H, h);
					childNode.set(F, f);
					openList.add(childNode);
				}else if(openList.contains(childNode) && childNode.get(seen) == 1){
					if(currG < childNode.get(G)){
...
class FComparer3 implements Comparator<SearchNode>{
	public int compare(SearchNode me, SearchNode them){
		float myF = me.get(MyCoolAgent_RWA_G.F);
		float theirF = them.get(MyCoolAgent_RWA_G.F);
